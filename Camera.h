...
class Camera
{
    ...
    public:
        ...
        __host__ __device__ inline Ray getRay(float u, float v) const; // Get the ray corresponding the u,v cordinates on the screen
};

__host__ __device__ Camera::Camera() {}

__host__ __device__ Camera::Camera(const Vector3& position, const Vector3& direction, const Vector3& up, float sx, float sy, float screen_distance) : p_e(position), v_view(direction), v_up(up), s_x(sx), s_y(sy) {
